...
// Fixed site clock: 2026-05-29 14:30 local in America/Edmonton (UTC-06 in
// May). All `when` instants below are expressed in UTC and chosen so their
// site-local reading lands deterministically in the bucket under test.
...
describe('bucketFor', () => {
    ...
    it('anchors the calendar-day boundary in the site timezone.', () => {
        ...
        expect(bucketFor('2026-05-29T05:30:00.000Z', NOW, TZ)).toBe('week');
    });
});

describe('formatAlertTimestamp', () => {
    it('formats new/today instants as 12-hour time of day.', () => {
        expect(formatAlertTimestamp('2026-05-29T20:02:00.000Z', NOW, TZ)).toBe('2:02 pm');
    });

    it('formats this-week instants with a weekday prefix.', () => {
        // 23:21 site-local on May 28 (a Thursday).
        expect(formatAlertTimestamp('2026-05-29T05:21:00.000Z', NOW, TZ)).toBe('Thu 11:21 pm');
    });

    it('formats older instants as a calendar date.', () => {
        expect(formatAlertTimestamp('2026-05-22T16:00:00.000Z', NOW, TZ)).toBe('May 22');
    });
});
